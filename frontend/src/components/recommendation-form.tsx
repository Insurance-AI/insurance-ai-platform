  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const cleanedData = {
        ...
        Existing_Loans_Debts: formData.Existing_Loans_Debts ? 1 : 0,
        Existing_Insurance_Policies: formData.Existing_Insurance_Policies ? 1 : 0,
      ...
      <Card className="p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <SelectField
              label="Existing Loan"
              name="Existing_Loans_Debts"
              value={String(formData.Existing_Loans_Debts)}
            ...
            />
            <SelectField
              label="Existing Insurance Policies"
              name="Existing_Insurance_Policies"
              value={String(formData.Existing_Insurance_Policies)}
              onChange={handleSelectChange}
              options={["true", "false"]}
            />
            <InputField
              label="Desired Sum Assured"
              name="Desired_Sum_Assured"
              type="number"
              value={formData.Desired_Sum_Assured}
              onChange={handleChange}
              required
            />
            <InputField
            ...
            <SelectField
              ...
              name="Premium_Payment_Option"
              value={formData.Premium_Payment_Option}
              onChange={handleSelectChange}
            ...
            <SelectField
              ...
              value={formData.Death_Benefit_Option}
              onChange={handleSelectChange}
            ...
            <SelectField
              ...
              onChange={handleSelectChange}
              options={["Hypertensive", "Healthy", "Diabetic"]}
            />
            <SelectField
            ...
            />
            <SelectField
              label="Interested in Optional Riders?"
              name="Interest_in_Optional_Riders"
              value={String(formData.Interest_in_Optional_Riders)}
              onChange={handleSelectChange}
              options={["true", "false"]}
            />
            <SelectField
              label="Interested in Tax Saving?"
              name="Interest_in_Tax_Saving"
              value={String(formData.Interest_in_Tax_Saving)}
              onChange={handleSelectChange}
              options={["true", "false"]}
            />
          </div>
