...
export default function SavingsOpportunities({ financialAdvice }: SavingsOpportunitiesProps) {
  ...
  const savingsPercent = avg_monthly_spending > 0 ? (potentialSavings / avg_monthly_spending) * 100 : 0;
  
  // Convert to array for rendering
  const savingsCategories = savings_opportunities ? 
    Object.entries(savings_opportunities).map(([name, data]) => ({
      name,
      ...data,
    })) : [];
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        {budget_recommendation && avg_monthly_spending && (
